...
def about(subject):
    ...
    def func(paragraph):
        for x in subject:
            if x in remove_punctuation(lower(paragraph)).split(" "):
                return True
        return False
    return func
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    total = 0
    result =min((len(typed_words)),len(source_words))
    if typed == '' and source== '':
        return 100.0
    ...
    for i in range(result):
        if typed_words[i]==source_words[i]:
            total =total+1
    answer= (total*100)/len(typed_words)
    return answer
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if limit<=0:
            ...
            result = float('inf')
            if len(typed)>0:
                result=min(result,memoized(typed[1:],source,limit-1)+1)
            if len(source)>0:
                ans= min(ans, memoized(typed,source[1:],limit-1) + 1)
            if typed and source and typed[0] ==source[0]:
                ans = min(ans, memoized(typed[1:],source[1:],limit))
        cache[(typed,source,limit)] =ans
        return ans
        # END PROBLEM EC
    return memoized
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    smalldif = [diff_function(typed_word, word, limit) for word in word_list]

    if typed_word in word_list:
        return typed_word
    if min(smalldif)> limit:
        return typed_word
    else:
        min(smalldif)
        return word_list[smalldif.index(min(smalldif))]
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    """
    """assert False, 'Remove this line'"""
    if not typed and not source:  # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 0
        # END
    ...
    if not typed or not source:
        return abs(len(typed)-len(source))
    if typed[0] == source[0]: #recursive
        return minimum_mewtations(typed[1:], source[1:], limit)
    else:
        add = minimum_mewtations(typed, source[1:], limit-1) #"ogs>-dogs" # Fill in these lines
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if i >= len(prompt) or (typed[i]!=prompt[i]):
            break
        result =result+1.0
    result/=len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times=[]
    tpp= times_per_player
    for i in range(len(tpp)):
        time = []
        for j in range(len(tpp[i])-1):
            time.append(tpp[i][j+1]-tpp[i][j]) #difference
        times.append(time)
...
def fastest_words(match):
    ...
    fastestplayer = [[] for _ in player_indices]

    for word_index in word_indices:
        time1 = [get_all_times(match)[player_index][word_index] for player_index in player_indices]
        fast = min(time1)
        player = [player_index for player_index, value in enumerate(time1) if value == fast]

        won = player[0]

        for player_index in player:
            if player_index == won:
                fastestplayer[player_index].append(get_all_words(match)[word_index])
